...
void tratar(char topic[], int message) {
	...
	if(!comp.automacaoTOGGLE){
		...
		// ALARME, TOGGLE, LIGADO E DESLIGADO
			...
			bool prox = !comp.alarme.alarmeAtivado;
			comp.alarme.alarmeAtivado = prox;
			MENSAGEM_RECEBIDA = MUDANCA_ALARME;
		}
		else if(strcmp(topic, TOPIC_ALARME_LIGADO) == 0){
			comp.alarme.alarmeAtivado = true;
			MENSAGEM_RECEBIDA = MUDANCA_ALARME;
		}
		else if(strcmp(topic, TOPIC_ALARME_DESLIGADO) == 0){
			comp.alarme.alarmeAtivado = false;
			MENSAGEM_RECEBIDA = MUDANCA_ALARME;
		}
	...
	if(MENSAGEM_RECEBIDA > 0){
		...
		if(MENSAGEM_RECEBIDA<= 6){
			if(MENSAGEM_RECEBIDA == 1){
				...
				snprintf (valueAux, 10, "%d", comp.alarme.alarmeAtivado);
				publishMessage(client, TOPIC_ALARME, valueAux);
				if(comp.alarme.alarmeAtivado == 1){
					// lcdClear(lcd);
					// lcdPuts(lcd, "Alarme:");
					// lcdPosition(lcd, 4, 1);
					// lcdPuts(lcd, "ligado");
  ...
  conn_opts.password = "";
	...
	while(true){
		...
		//////////////////////////////////////////////////////////////////////////
            ...
            comp.alarme.alarmeAtivado = comp.estados_inputs.alarme;
			//atualizarMongo("alarme_ligado", 1, comp.alarme.alarmeAtivado);
			...
			if(estadoCronometroAC){
				...
				if(now > desligar_em && desligar_em > 0){
					...
					if(returnAC == 1){
                        ...
                        printf("TEMPERATURA DO AR CONDICIONADO ALTERADO PARA %d.\n", comp.ac.temp_atual);
					}
                ...
                printf("Tem pessoas na sala do ar condicionado? %d\n", temPessoas_AC);
				if(returnAC == 1){
					snprintf (valueAux, 10, "%d", 0);
					publishMessage(client, TOPIC_ARCONDICIONADO, valueAux);
					estadoCronometroAC = true;
					time_t agora = time(0);
					desligar_em = agora + 1*60;
					printf("Ar condicionado ficará desligado por 1 minutos por estar fora da faixa de operacao.\n");
				} else if(returnAC == 2){
					estadoCronometroAC = true;
					time_t agora = time(0);
					desligar_em = agora + comp.ac.tempo_ausencia_pessoas*60;
					printf("Ar condicionado ficará ligado por mais %d minutos e sera desligado pela ausencia de pessoas no ambiente.\n", comp.ac.tempo_ausencia_pessoas);
				}
			}
			if(ALTERACAO_LOGS){
				atualizarLogsLocal();
